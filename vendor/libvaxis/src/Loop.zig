...
pub fn Loop(comptime T: type) type {
    return struct {
        ...
        const Event = T;
        const ActiveResizeLoop = if (builtin.os.tag == .windows) struct {} else struct {
            var value: std.atomic.Value(?*Self) = .init(null);
        };
        const ResizeSignalState = if (builtin.os.tag == .windows) struct {} else struct {
        ...
        // SIGWINCH is asynchronous. Never touch the event queue from the
        // signal handler: Queue.push() takes an std.Io.Mutex and can deadlock
        // if the signal interrupts code that already owns that mutex.
        resize_pipe: ?[2]std.posix.fd_t = null,
        resize_pending: std.atomic.Value(bool) = .init(false),
        resize_signal: ResizeSignalState = .{},
        ...
        pub fn installResizeHandler(self: *Self) !void {
            switch (builtin.os.tag) {
                .windows => {},
                    ...
                    if (builtin.is_test or self.resize_handler_installed) return;
                    try self.ensureResizePipe();

                    var action = std.posix.Sigaction{
                        .handler = .{ .handler = Self.handleWinchSignal },
                    ...
                    ActiveResizeLoop.value.store(self, .release);
                    std.posix.sigaction(std.posix.SIG.WINCH, &action, &previous);
                    self.resize_signal.previous_action = previous;
        ...
        pub fn uninstallResizeHandler(self: *Self) void {
            switch (builtin.os.tag) {
                .windows => {},
                    ...
                    if (builtin.is_test or !self.resize_handler_installed) return;
                    if (self.resize_signal.previous_action) |*previous| {
                        std.posix.sigaction(std.posix.SIG.WINCH, previous, null);
                    }
                    ActiveResizeLoop.value.store(null, .release);
                    self.resize_signal.previous_action = null;
                    self.resize_handler_installed = false;
                },
            }
        }

        fn handleWinchSignal(_: std.posix.SIG) callconv(.c) void {
            const self = ActiveResizeLoop.value.load(.acquire) orelse return;
            Self.winsizeCallback(self);
        }
        ...
        pub fn start(self: *Self) !void {
            ...
            switch (builtin.os.tag) {
                .windows => {},
                else => if (@hasField(Event, "winsize")) try self.ensureResizePipe(),
            }
            errdefer self.closeResizePipe();
        ...
        pub fn stop(self: *Self) void {
            ...
            // Wake both the legacy blocking input path and the POSIX poll path.
        ...
        pub fn winsizeCallback(ptr: *anyopaque) void {
            ...
            if (self.vaxis.state.in_band_resize or !@hasField(Event, "winsize")) return;

            const pipe = self.resize_pipe orelse return;
            // Coalesce a resize storm to at most one outstanding pipe byte.
            if (self.resize_pending.swap(true, .acq_rel)) return;
            const byte = [_]u8{1};
            const rc = std.posix.system.write(pipe[1], &byte, 1);
            if (std.posix.errno(rc) != .SUCCESS) {
                // Allow a later SIGWINCH to retry if the async-safe write failed.
                self.resize_pending.store(false, .release);
            }
        }

        fn ensureResizePipe(self: *Self) !void {
            if (self.resize_pipe != null) return;
        ...
        fn closeResizePipe(self: *Self) void {
            const pipe = self.resize_pipe orelse return;
            var read_file: std.Io.File = .{ .handle = pipe[0], .flags = .{ .nonblocking = false } };
            var write_file: std.Io.File = .{ .handle = pipe[1], .flags = .{ .nonblocking = false } };
            read_file.close(self.io);
            write_file.close(self.io);
            self.resize_pipe = null;
            self.resize_pending.store(false, .release);
        }
        ...
        fn _ttyRun(
            ...
            switch (builtin.os.tag) {
                .windows => {
                    ...
                    var parser: Parser = .{};
                    const resize_pipe = self.resize_pipe;
                    ...
                    read_loop: while (!self.should_quit) {
                        if (resize_pipe) |pipe| {
                            ...
                            if (poll_fds[1].revents & std.posix.POLL.IN != 0) {
                                ...
                                _ = std.posix.system.read(pipe[0], &byte, 1);
                                if (self.resize_pending.swap(false, .acq_rel)) {
                                    const new_winsize = try self.tty.getWinsize();
                                    try self.postEvent(.{ .winsize = new_winsize });
                                }
                            }
                            if (self.should_quit) continue :read_loop;
                            if (poll_fds[0].revents & std.posix.POLL.IN == 0) continue :read_loop;
                        }
                        ...
                        while (seq_start < n) {
                            if (n - seq_start == 1 and buf[seq_start] == 0x1b) {
                                ...
                                var esc_poll_fds = [1]std.posix.pollfd{.{
                                    .fd = self.tty.fd.handle,
                                    .events = std.posix.POLL.IN,
                                    .revents = 0,
                                }};
                                _ = std.posix.poll(&esc_poll_fds, 0) catch 0;
                                if (esc_poll_fds[0].revents & std.posix.POLL.IN != 0) {
                                    buf[0] = buf[seq_start];
                                    read_start = 1;
                                    continue :read_loop;
                                }
        ...
        .windows => {
            ...
            switch (event) {
                ...
                .winsize => |winsize| {
                    ...
                    switch (builtin.os.tag) {
                        .windows => {},
                        // In-band resize supersedes the SIGWINCH fallback.
